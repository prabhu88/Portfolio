...
import {
    ...
    TableHead, TableBody, TableRow, TableCell, Card, CardContent,CardHeader
} from '@material-ui/core'
import * as XLSX from "xlsx";
import '../B2b_recon.scss'
import Matched from './Matched'
import Edata from './Books - Template.xlsx'
...
const Gstr2A_recon = () => {
    ...
    const ProcessData = async () => {
        try {
            ...
            let b2b = Json_record.b2b != undefined ? Json_record.b2b : [] //Json_record.data.docdata.b2b != undefined ? Json_record.data.docdata.b2b : [] 
            b2b.map(({ inv, ctin, fldtr1, flprdr1, cfs, cfs3b }, i) => {
                inv.map(({ inum, val, inv_typ, pos, idt, rchrg, itms }, j) => {
                    let INV_3b = []
                    INV_3b.length = 0
                    INV_3b['gstin'] = ctin
                    INV_3b['filled_date'] = fldtr1
                    INV_3b['filled_periold'] = flprdr1
                    INV_3b['inv_no'] = inum
                    INV_3b['inv_date'] = idt
                    INV_3b['pos'] = pos
                    INV_3b['inv_typ'] = inv_typ
                    ...
                    var local_taxval = 0
                    itms.forEach((e) => {
                        local_cgst += e.itm_det.camt ? e.itm_det.camt : 0
                        local_sgst += e.itm_det.samt ? e.itm_det.samt : 0
                        local_igst += e.itm_det.iamt ? e.itm_det.iamt : 0
                        local_cess += e.itm_det.cess ? e.itm_det.cess : 0
                        local_taxval += e.itm_det.txval ? e.itm_det.txval : 0
                    })
                    ...
                    books_record.forEach((data) => {
                        ...
                        if (gstin === ctin && inum === (data.inv).trim()) {
                            ...
                            let InvM_3b = []
                            InvM_3b['gstin'] = ctin
                            InvM_3b['filled_date'] = fldtr1
                            InvM_3b['filled_periold'] = flprdr1
                            InvM_3b['inv_no'] = inum
                            InvM_3b['inv_date'] = idt
                            InvM_3b['pos'] = pos
                            InvM_3b['inv_typ'] = inv_typ
                            ...
                            InvM_3b['Books_inv_cgst'] = data.cgst != '' ? data.cgst : 0
                            InvM_3b['Books_inv_sgst'] = data.sgst != '' ? data.sgst : 0
                            InvM_3b['Books_inv_igst'] = data.igst != '' ? data.igst : 0
                            InvM_3b['Books_inv_cess'] = data.cess != '' ? data.cess : 0
                            let diff ,diff_cgst,diff_sgst,diff_igst,diff_cess  = 0
                            diff = Math.floor(val)-Math.floor(data.inv_total)
                            diff_cgst = Math.floor(local_cgst) -Math.floor(data.cgst)
                            diff_sgst = Math.floor(local_sgst) -Math.floor(data.sgst)
                            diff_igst = Math.floor(local_igst) -Math.floor(data.igst)
                            diff_cess = Math.floor(local_cess) -Math.floor(data.cess)
                            InvM_3b['Diff_total'] = diff
                            InvM_3b['Diff_cgst'] = diff_cgst
                            InvM_3b['Diff_sgst'] = diff_cgst
                            InvM_3b['Diff_igst'] = diff_cgst
                            InvM_3b['Diff_cess'] = diff_cgst
